...
int test_visanchors()
{
	...
	for (int i = 0; i < solver.m_anchor_lib.anchors.size(); i++)
	{
		...
		std::stringstream ss;
		ss << "D:/pig_middle_data/build_pose_lib5/pose_lib5_obj/pose" << i << ".obj";
		solver.saveObj(ss.str()); 
		...
		std::stringstream outss;
		outss << "D:/pig_middle_data/build_pose_lib5/pose_lib5_render/anchor_" << i << ".png";
		cv::imwrite(outss.str(), img);
