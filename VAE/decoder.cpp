...
Decoder::Decoder()
{
	...
	p_dec_out = std::make_shared<LinearLayer>(512, 62 * 6);
	//p_dec_tanh = std::make_shared<Tanh>(); 
	p_dec_lrelu1 = std::make_shared<LeakyReLU>(0.2); 
	p_dec_lrelu2 = std::make_shared<LeakyReLU>(0.2); 
	p_dec_rot = std::make_shared<ContinousRotation>();

	std::string folder = "F:/projects/model_preprocess/designed_pig/pig_prior/params_txt/";
...
void Decoder::forward()
{
	...
	p_dec_rot->input = p_dec_out->output;

	//p_dec_tanh->forward(); 
	p_dec_rot->forward(); 
	output = p_dec_rot->output; 
}
