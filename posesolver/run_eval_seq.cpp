...
void  run_eval_seq()
{
	...
	std::string config_file = get_config("eval_config");
	FrameSolver frame;
	...
	for (int k = start; k < start + framenum; k++)
	{
		...
		//if(frameid == start && frame.m_use_init_cluster)
		//	frame.load_clusters();
		//else if (frameid == start) {
		//	frame.matching_by_tracking(); 
		//}
		//else frame.pureTracking(); 
		//frame.save_clusters(); 

#if 1 // run direct triangulation 
		frame.load_clusters(); 
