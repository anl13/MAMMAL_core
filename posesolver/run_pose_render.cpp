...
void run_trajectory2()
{
	...
	cv::VideoWriter writer("D:/paper_writing_figs/stand.avi", cv::VideoWriter::fourcc('m', 'p', 'e', 'g'), 25.0, cv::Size(1920, 1080));
	if (!writer.isOpened())
	{
		std::cout << "not open" << std::endl;
		return;
	...
	FrameSolver frame;
	std::string configfile = "configs/config_20190703_motion2.json";
	frame.configByJson(conf_projectFolder + configfile);
	frame.init_parametric_solver();
	auto solvers = frame.get_solvers();
	int start = 3248;
