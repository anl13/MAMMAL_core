...
void RenderObject::SetMaterial(const MaterialParam& _materialParam)
{
	materialParam = _materialParam;
}


void RenderObject::SetFaces(const Eigen::Matrix<unsigned int, 3, -1, Eigen::ColMajor>& faces, const bool inverse)
{
	faceNum = faces.cols();
...
void RenderObject::SetVertices(const Eigen::Matrix<float, 3, -1, Eigen::ColMajor>& vertices)
{
	glBindVertexArray(VAO);
...
void RenderObject::SetTransform(const Eigen::Vector3f& _translation, const Eigen::Vector3f& _rotation, const float _scale)
{
...
void RenderObjectColor::DrawWhole(Shader& shader) const
{
	shader.SetMat4("model", model);
...
void RenderObjectTexture::DrawWhole(Shader& shader) const
{
	shader.SetMat4("model", model);
	shader.SetFloat("material_ambient", materialParam.ambient);
	shader.SetFloat("material_diffuse", materialParam.diffuse);
...
void RenderObjectMesh::DrawWhole(Shader& shader) const
{
	shader.SetMat4("model", model);
...
void BallStickObject::Draw(Shader& shader)
{
	for (RenderObjectColor* ptr : objectPtrs)
	{
